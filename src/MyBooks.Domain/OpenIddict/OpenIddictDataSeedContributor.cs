...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        if (await _openIddictScopeRepository.FindByNameAsync("MyBooks") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor {
                Name = "MyBooks", DisplayName = "MyBooks API", Resources = { "MyBooks" }
            });
        }
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        ...
        //Web Client
        var webClientId = configurationSection["MyBooks_Web:ClientId"];
        if (!webClientId.IsNullOrWhiteSpace())
        {
            var webClientRootUrl = configurationSection["MyBooks_Web:RootUrl"]!.EnsureEndsWith('/');

            /* MyBooks_Web client is only needed if you created a tiered
            ...
            await CreateApplicationAsync(
                ...
                secret: configurationSection["MyBooks_Web:ClientSecret"] ?? "1q2w3e*",
        ...
        // Swagger Client
        var swaggerClientId = configurationSection["MyBooks_Swagger:ClientId"];
        if (!swaggerClientId.IsNullOrWhiteSpace())
        {
            var swaggerRootUrl = configurationSection["MyBooks_Swagger:RootUrl"]?.TrimEnd('/');
        ...
        // App Client
        var appClientId = configurationSection["MyBooks_App:ClientId"];
        if (!appClientId.IsNullOrWhiteSpace())
        {
                await CreateApplicationAsync(
                    ...
                    secret: configurationSection["MyBooks_App:ClientSecret"] ?? "1q2w3e*",
